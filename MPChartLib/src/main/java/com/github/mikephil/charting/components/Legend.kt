...
class Legend() : ComponentBase() {
     ...
     */
     var extraEntries: Array<LegendEntry>? = null
        private set
    ...
    fun setExtra(entries: List<LegendEntry>) {
        extraEntries = entries.toTypedArray<LegendEntry>()
    }
    ...
    fun setExtra(colors: IntArray, labels: Array<String?>) {
        ...
        for (i in 0 until min(colors.size.toDouble(), labels.size.toDouble()).toInt()) {
            val entry = LegendEntry()
            entry.formColor = colors[i]
            entry.label = labels[i]

            if (entry.formColor == ColorTemplate.COLOR_SKIP ||
                    entry.formColor == 0) entry.form = LegendForm.NONE
        ...
        when (orientation) {
            LegendOrientation.VERTICAL -> {
                ...
                while (i < entryCount) {
                    ...
                    val formSize = if (java.lang.Float.isNaN(e.formSize)
                    ) defaultFormSize
                    ...
                    if (label != null) {
                        ...
                        if (drawingForm && !wasStacked) width += formToTextSpace
                            ...
                            maxWidth = max(maxWidth.toDouble(), width.toDouble()).toFloat()
            ...
            LegendOrientation.HORIZONTAL -> {
                ...
                while (i < entryCount) {
                    ...
                    if (label != null || i == entryCount - 1) {
                        ...
                        if (// It simply fits
                                !wordWrapEnabled || currentLineWidth == 0f || (contentWidth - currentLineWidth >=
                            ...
                            maxLineWidth = max(maxLineWidth.toDouble(), currentLineWidth.toDouble()).toFloat()

                            // Start a new line
                            mCalculatedLabelBreakPoints[if (stackedStartIndex > -1) stackedStartIndex
                            else i] = true
                ...
                mNeededHeight = (labelLineHeight
                        * mCalculatedLineSizes.size.toFloat()
                        + labelLineSpacing * (if (mCalculatedLineSizes.size == 0
                ) 0
                else (mCalculatedLineSizes.size - 1)).toFloat())
            }
        }
